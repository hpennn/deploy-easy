...
应用将加载在线 URL（https://deploy.zhinenti.cn），网络不可用时自动降级到 `frontend/index.html`。
...
```
dist-electron/
├── 智能部署助手-1.0.0-Setup.exe        # Windows NSIS 安装包
├── 智能部署助手-1.0.0-x64.dmg          # macOS Intel DMG
├── 智能部署助手-1.0.0-arm64.dmg        # macOS Apple Silicon DMG
└── 智能部署助手-1.0.0-x86_64.AppImage  # Linux AppImage
```

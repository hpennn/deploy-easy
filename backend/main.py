...
async def download_apk():
    ...
    if os.path.isfile(apk_path):
        return FileResponse(apk_path, media_type="application/vnd.android.package-archive", filename="智能部署助手-v1.0.apk")
...
async def download_exe():
    ...
    if os.path.isfile(file_path):
        return FileResponse(file_path, media_type="application/octet-stream", filename="智能部署助手.exe")
    from fastapi.responses import JSONResponse
...
async def download_dmg():
    ...
    if os.path.isfile(file_path):
        return FileResponse(file_path, media_type="application/octet-stream", filename="智能部署助手-macOS.tar.gz")
    from fastapi.responses import JSONResponse

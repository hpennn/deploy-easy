...
(function() {{
    ...
    chatWindow.id = 'deploy-easy-chat-window';
...
async def download_apk():
    ...
    if os.path.isfile(apk_path):
        return FileResponse(apk_path, media_type="application/vnd.android.package-archive", filename="智能部署助手.apk", headers={"accept-ranges": "bytes"})
...
async def download_exe():
    ...
    if os.path.isfile(exe_path):
        return FileResponse(exe_path, media_type="application/octet-stream", filename="智能部署助手.exe", headers={"accept-ranges": "bytes"})
    from fastapi.responses import JSONResponse
...
async def download_dmg():
    ...
    if os.path.isfile(dmg_path):
        return FileResponse(dmg_path, media_type="application/octet-stream", filename="智能部署助手-mac.tar.gz", headers={"accept-ranges": "bytes"})
    from fastapi.responses import JSONResponse
